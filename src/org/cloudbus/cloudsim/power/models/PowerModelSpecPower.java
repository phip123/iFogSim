...
public abstract class PowerModelSpecPower implements PowerModel {
	...
	public double getPower(Resources utilization) throws IllegalArgumentException {
		...
		if (cpuUtil % 0.1 == 0) {
			return getPowerData((int) (cpuUtil * 10));
		}
		int utilization1 = (int) Math.floor(cpuUtil * 10);
		int utilization2 = (int) Math.ceil(cpuUtil * 10);
		double power1 = getPowerData(utilization1);
		double power2 = getPowerData(utilization2);
		double delta = (power2 - power1) / 10;
		double power = power1 + delta * (cpuUtil - (double) utilization1 / 10) * 100;
		return power;
	}
