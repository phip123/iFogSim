...
public class Controller extends SimEntity {
    ...
    private double calculateDistance(FogDevice fogDevice, FogDevice anUpperDevice) {
        return Math.sqrt(Math.pow(fogDevice.getxCoordinate() - anUpperDevice.getxCoordinate(), 2.00) + Math.pow(fogDevice.getyCoordinate() - anUpperDevice.getyCoordinate(), 2.00));
    }
    ...
    private void formClusters() {
        ...
        for (int i = 0; i < getFogDevices().size(); i++) {
            ...
            for (int j = 0; j < getFogDevices().size(); j++) {
                ...
                if (fd1.getId() != fd2.getId() && fd1.getParentId() == fd2.getParentId() && calculateDistance(fd1, fd2) < Config.CLUSTER_DISTANCE && fd1.getLevel() == fd2.getLevel()) {
                    ...
                    } else if (fd1ClusteriD == -1) clusterInfo.put(fd1.getId(), clusterInfo.get(fd2.getId()));
                    else if (fd2ClusteriD == -1) clusterInfo.put(fd2.getId(), clusterInfo.get(fd1.getId()));
                }
            }
        }
        ...
        for (int id : clusters.keySet()) System.out.println(id + " " + clusters.get(id));
    }
